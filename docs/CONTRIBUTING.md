...
Coverage is measured automatically on every `make test` run and printed to the terminal. An HTML report is also written to `htmlcov/`: open `htmlcov/index.html` in a browser to browse line-by-line.

```bash
make coverage          # run tests + show coverage in terminal
```

**What's measured:** all code under `app/`. Migrations are excluded (autogenerated). `TYPE_CHECKING` blocks are excluded (never execute at runtime).

`htmlcov/` is gitignored — it's a build artifact, not source.
